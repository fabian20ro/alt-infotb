...
### [2026-09-25] Investigate reported missing N109 markers

**Context:** User sees roughly two thirds of N109 stops and asks whether a marker limit exists.
**Findings:** PR #44 merged on 2026-09-24. Current code retains selected-line priority stops when the general station layer exceeds 100 markers. TPBI 6.39 lists 16 stops in direction 0 and 17 in direction 1, with 32 unique IDs; all 32 are present in the bundle. Calling the viewport filter with citywide bounds, all 2,525 stations, and N109 selected returns all 32 with none missing. Nine stop pairs are less than 100 meters apart, so overlapping markers are a plausible visual explanation, not a confirmed reproduction of the user's view.
**Outcome:** No application changes. Need the selected station and an example missing stop to distinguish overlap, viewport bounds, installed version, or live-route/catalog differences.
**Promoted to Lessons Learned:** No.

### [2026-09-25] Isovolta confirms incomplete N109 membership

**Context:** User identified Isovolta as a missing N109 stop.
**Evidence:** Bundled Isovolta stop 6084 lists BUS:103 and BUS:246 but omits BUS:N109. A successful protobuf response from the production proxy `/lines/stop?stop_id=6084` (using the application's allowed Origin) explicitly lists N109, line ID 199, direction 1 toward Piata Unirii 2. The STB announcement https://www.stb.ro/info?N=2097 describes the extension to Isovolta effective 2024-05-27, with eleven extension stops in both directions; TPBI's public network page also lists Isovolta as N109's terminus. The current GTFS download still has Last-Modified 2026-09-09T19:15:24Z, matching the bundled feed.
**Conclusion:** The previous 32/32 check established internal consistency, not route completeness. Isovolta is wrongly excluded from N109's priority set; the general 100-marker density filter can therefore hide it. This example is a confirmed source-membership gap, not evidence of marker overlap. No application code changed during diagnosis.
**Promoted to Lessons Learned:** Yes.

### [2026-09-25] Broader surface-stop membership audit

**Context:** User asked whether other stations/lines have similar problems.
**Method:** Queried 31 purposively selected surface stop IDs sequentially through the production proxy; decoded binary responses with the existing ProtoReader. 30 responses contained named station/line data; ID 6014 was empty and remains unverified. Counted only positive live memberships absent from the catalog. A read-only architect agent independently reviewed the comparison path and recounted discrepancies.
**Findings:** 60 unmatched station–line pairs across 23 stop IDs and 39 lines. Of these, 25 pairs (13 stops, 13 trolleybus lines) are a code-level CABLE_CAR/TROLLEYBUS identifier mismatch. After accounting for the alias, 35 pairs remain missing at 14 stops across 26 lines, of which 23 have no membership in the entire bundled catalog. Confirmed examples include 640 at Isovolta, N103 at Bucur Obor, 47 at Piata Unirii, N700 at Piata Unirii 1/2/5, and 403/447/447B at Piata Presei. This biased sample cannot estimate prevalence across the whole network.
**Outcome:** Report saved in docs/station-membership-audit-2026-09-25.md; raw local snapshots retained in /tmp/better-stb-membership-audit-20260925. No application code changed. Wider source gaps and transport-type normalization need separate remediation.
**Surprise:** The API encodes observed trolleybuses as CABLE_CAR; the prior mocked tests supplied TROLLEYBUS, so they failed to exercise the real API/catalog boundary.
**Promoted to Lessons Learned:** Yes.

### [2026-09-25] Plan exhaustive membership regression checks and catalog architecture

**Context:** User requested an automatic all-stations/all-lines regression test and an architectural implementation recommendation covering future discrepancies.
**Research:** Inspected the official InfoTB client's public methods and embedded protobuf schema. It uses a line registry, line detail and directional detail endpoints; line detail includes stop IDs and coordinates. These endpoints have not yet been verified live through our proxy, which currently allows only the stop endpoint. Client/schema evidence establishes a candidate source, not network completeness.
**Recommendation:** Keep the static PWA and versioned bundled catalog. Validate STB topology first, then generate memberships by stable STB line IDs with explicit source provenance and GTFS fallback coverage. Add deterministic exhaustive source-to-candidate tests to every PR, a daily topology audit and a weekly/manual all-known-API-stop positive-evidence audit. Treat incomplete captures as inconclusive; retain the last validated catalog on upstream failure. Require explicit coverage accounting, metro platform resolution, bounded resumable collection and guarded removals.
**Deliverable:** docs/plans/2026-09-25-station-catalog-audit.md specifies stages, files, verification, rollout and acceptance criteria. Tests, proxy extensions, application changes and recurring workflows remain proposed, not implemented or scheduled in this planning iteration.
**Verification:** Checked the plan against current proxy, catalog generator, map filter and deployment workflow. git diff --check passed; application tests not rerun for documentation-only work.
**Surprise:** Current scheduled deployment can skip all checks when GTFS-derived stations.json is unchanged; the independent live audit must run outside that condition.
**Promoted to Lessons Learned:** No additional lesson; previously recorded source-completeness and transport-alias findings apply.

### [2026-09-25] Implement STB topology catalog and exhaustive regression audit

**Context:** User approved the architectural recommendation, requested implementation, sufficient verification, a PR and an updated next plan including leftovers and an architectural improvement.
**Implementation:** Shared strict dev/production proxy for registry/detail/direction endpoints; coordinated auth refresh, request cancellation and auth-body deadlines. Pure custom protobuf decoder and canonical transport normalizer preserve raw identifiers. Serial resumable collector records response hashes and capture intervals, enforces budgets/retries and Romanian 04:00 freshness. Independent versioned STB/GTFS sources compose 3,911 physical markers using native STB line IDs. Existing marker IDs/favorites preserved; explicit M5/Tudor Arghezi mappings added; supported regional stations survive runtime loading. Exhaustive source-to-runtime verification exercises 203 lines, 406 directions and 9,759 direction edges through the real map density filter. Separate daily/weekly/manual audit workflow and fail-closed publication gate replace the GTFS-only scheduled overwrite path.
**Live evidence:** Complete registry topology contains 203 lines and 3,966 API stops; N109 has 55 distinct stops versus GTFS32 and includes all three Isovolta IDs. Full audit requested all 3,980 known API IDs: 3,966 named responses, 14 empty GTFS-only IDs. Strict final classification: 3,936 verified, 44 inconclusive (14 empty + 30 stops with unregistered services), 29 missing N700 memberships. Registry-absent service IDs 1036(N700),907(429),909(476) all return empty200 for detail/bothdirections. At 6207 old/listed and unregistered same-name IDs coexist across directions; no name-based alias invented. All 20 M5 platforms return line data. No directional contradictions for registered lines.
**Verification:** Svelte check: 0 errors/warnings; dedicated catalog TypeScript check and shared-proxy TypeScript check pass. Root suite: 948 tests / 22 files pass; shared proxy: 58 contract tests separately pass. Production build passes. Browser suite: 31 passed / 5 device-specific skips, workers: 1, no retries, Chromium desktop/mobile and mobile WebKit; includes real CABLE_CAR protobuf→66 selection→all 36 markers. Initial sandbox browser launches were blocked by macOS; rerun outside sandbox passed. Complete source-to-candidate CLI verification passes; attempting protected publication with the real inconclusive audit correctly fails. YAML parsing and git diff whitespace checks pass. Temporary server configuration not included in changed files.
**Surprises fixed during review:** Shared auth could hang while reading JSON; collector deadline could expire during pacing or body read; malformed cache bodies needed re-fetch; cached data could cross the 4 AM boundary; semantic-unchanged candidates needed new provenance; upstream identity must be separate from proxy URL; Node scripts needed a dedicated typecheck beyond Svelte. Added regression coverage for these boundaries.
**Limitations and rollout:** Public proxy promotion remains separate (WORKER_DEPLOY_ENABLED=false, no repository deployment secrets). Recurring audit workflow is delivered in the PR, not yet active on main. Automatic publication remains disabled and will reject the unresolved upstream differences. Bundled initial migration is explicitly reviewed for listed registry coverage; no claim of universal STB completeness. No production deployment, merge, credentials update or name-based service alias performed.
**Next plan:** docs/plans/2026-09-25-station-catalog-next.md lists deployment activation, upstream incidents, fourteen legacy IDs and future source-inventory closure. Recommend explicit physical-station/platform/service identities and bounded discovery from positive stop observations, with provenance and expiring exact exceptions only after review.
**Promoted to Lessons Learned:** Yes — registry omissions, separate upstream/transport identities, newly available M5, dedicated Node tooling typecheck.

### [2026-09-25] Final identity review and push-destination recovery

**Review correction:** A registry-unknown service could still match compatibility names rebuilt from STB. Separated `fallbackLines` from display/compatibility keys; unknown STB IDs may use only explicitly supplemental GTFS memberships, never another STB service's name. Added runtime and publication mutation regressions. Final strict full-audit comparison now reports 31 missing memberships: 29 N700 plus IDs907/909 at6207; coverage remains3936verified/44inconclusive out of3980. Evidence was re-evaluated from the retained capture with fresh probes for empty responses and rebound to the final catalog hash. Final unit total950.
**Push incident:** `push.default=upstream` plus a new local branch tracking origin/main made `git push -u origin codex/stb-catalog-audit` publish commit2b9f062 to main, instead of creating a PR branch. Immediately informed user, cancelled Deploy run36076508478 before artifact upload (frontend/worker jobs cancelled without steps), and restored main with revert0617b71. Verified its tree equals previous main d2f85d5. Cancelled the revert-triggered Deploy too. No force push or other contributor changes. Reapplied implementation on the restored base for a clean PR; subsequent push uses an explicit destination refspec.
...
### [2026-09-25] Implement historical service inventory and bounded discovery

**Request:** Implement the recommended next plan; preserve the user's two dependency updates. Started a no-track feature branch from origin/main after PR45 merged, keeping devalue5.9.2 and nanoid3.3.19. Carried forward the earlier plan clarification that missed the PR45 merge cutoff.
**Implementation:** Independent historical service inventory, evidence/hash/source validation, truthful response timestamps, registry-only/full-scan assessment and per-ID label/history summaries. Operating state stays unknown; same-name IDs are separate. Discovery follows positive stop observations through fully verified detail/both directions/new regional stops, with caps, budgets, stable registry and Bucharest04:00 freshness. Diagnostic discoveries do not alter the provider registry/runtime catalog. Dedicated data/service-inventory branch persists validated append-only history independently of strict catalog publication, with atomic restores, exact-ref compare-and-swap writes and no worktree/index mutation. Workflow retains useful observations before surfacing inconclusive scans. Automatic catalog publication remains disabled.
**Evidence:** Replayed all3980 retained raw stop responses against source hashes and the bundled snapshot:3936verified/44inconclusive;206service identities, including unlisted907/909 atone stop each and1036 at29stops. Replay is historical, not a fresh live capture. Registry-only CLI smoke passes. Both user dependency updates confirmed installed by npm ls.
**Verification:** 1049 root unit tests/26files, including99new tests across history/capture/discovery/realGit storage;455catalog tests. Svelte check0errors/warnings, catalog/proxyTypeScript and productionbuild pass. Storage tests include actual CLI roundtrip and concurrent writers with bare repositories. YAML parsing and diff whitespace checks pass. Final browser result recorded below after isolated run.
**Proxy promotion:** Public /lines probe404. Prepared private shared-api-host PR2 with immutable public main pin and isolated provider builds. Its former all-module build failed cloning an unrelated unavailable repository; selected-module build fixes this without changing Render composition. Host TypeScript,3tests, exact-source Node/Worker artifact build and Wrangler dryrun pass.58module contracttests pass with standalone test tsconfig, now added to public shared-api/test so test execution no longer depends on generated frontend configuration. No deployment performed: GitHub deploymenttoken absent and local WranglerOAuth expired; requested user reauthentication asynchronously.
**Surprises:** Registry recheck evidence overwrites initial response proof when registry changes; normalizer now excludes uncertain registry positives. Root Vitest startup regenerates Svelte client files, causing full-browser reload if E2E runs concurrently; reproduced at the exact timestamp and isolated from UI logic. No speculative map change. Initial npm ci could not write sandboxed default cache; successful retry used explicit /tmp cache, with lockfile unchanged.
**Follow-up:** docs/plans/2026-09-25-service-inventory-next.md records rollout, unresolved upstream services/14legacyIDs, GTFS fallback limits, retention policy and a future explicit contract for admitting discovered services into a public catalog. Current history is not an operational calendar or a live service-state oracle.
