...
## Tasks
- [ ] **Task 1: Update `src/lib/api/client.test.ts`**
    - Add a test case for HTTP 400 response, verifying the generic error message is handled correctly.
    - Add a test case for HTTP 500 response, verifying it also uses the standard error format.
- [ ] **Task 2: Verification**
    - Run `npm test -- src/lib/api/client.test.ts` and ensure all tests (existing + new) pass.
